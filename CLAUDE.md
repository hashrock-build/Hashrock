...
- 🟦 **M3** *(current)* — Authoritative server + on-chain deposit/redeem. Stack: **Colyseus** (WS) + **Postgres** + treasury = **EOA** (no custom program for MVP; multisig/program at mainnet per invariant #5).
  ...
  - ✅ **3b**: **on-chain redeem + deposit** — $HASHROCK SPL on devnet (decimals 0, fixed 1B, mint authority burned), treasury = EOA. `chain.ts` redeemTo/verifyDeposit; DB persistRedeem/refund/persistDeposit (sig-deduped, transactional). Client Profile: Phantom/Backpack connect, save wallet, redeem popup (burn→release, refund on fail). **Blockhash relayer**: `chain.startBlockhashRelayer()` polls the latest devnet blockhash; `spawnOre` derives the cell from it (`blockhashValue` = last 2 bytes). Profile cosmetics: outfit tint + axe rarity (hair/hat slots dormant — need custom sprite sheets to render/sell). Setup: `server/scripts/setup-chain.mjs` (secret in `server/.treasury.json`, gitignored; mint+treasury in `.env`). **Anti-cheat**: server validates move speed (×1.8 tol) + collision map. **Deposit auto-watcher**: polls treasury, maps sender→player, auto-credits (dedupe by sig). **On-chain axe purchase**: server builds transfer → wallet signs → server verifies + grants axe + routes 95% pool/5% creator (axeOwned gates equip). Left: cosmetic sprite-sheet art (procedural looked bad — needs real per-direction sheets), secondary player↔player marketplace, mainnet (treasury multisig/program).
  - Characters: 2 selectable bodies (Body_A "Rookie" + A_Hunter from the Pixel Crawler bundle, drop-in 4-dir). Axe rarity icons = recolored sprite from the Wood weapon sheet (`public/assets/axes/axe_0..4.png`). NOTE: Pixel Crawler art = gameplay only, NOT sellable/NFT.
  - Run: `./server/scripts/pg.sh start` → `npm --prefix server run dev` → `npm run dev` (client).
- ⬜ **M4** — End-to-end Phantom, blockhash relayer, live testnet; secondary marketplace, repair/durability sink, character preview UI.
- ⬜ **M5 (post-mainnet)** — **Themed mining zones** using the Pixel Crawler environment bundle (Cave/Desert/Forge/Garden/Library/Sewer/etc. tilesets + props) — variety of biomes/maps. Queued after mainnet.
